...
def write_scan_dir(
    ...
    TODO: L1 에 열 목록 상수가 생기면(`msa.l1.scoreboard` 는 `BLOCKS` 만 내놓는다) 그것으로 쓴다 —
    지금은 `load_scorecard` 가 읽는 열만 손으로 맞춘다.
    """
...
def valid_thesis() -> dict[str, Any]:
    """스키마·§4 규약을 전부 만족하는 thesis — `conftest.make_thesis` 에 L3 검증기 테스트가 깨뜨릴
    표적(bear 원문 · high/medium/low + 12개월 초과 증거)만 얹는다."""
    return make_thesis(
        generated_at=ASOF,
