...
| **일간** | **후보 다이제스트 + 무효화 점검** (기계) — 상위 K 테마·테마별 명단 상위 N·직전 대비 변화 · 사람은 **고르는 것·저널**만. **L3(LLM)는 돌지 않는다** (§1.1) | 0~5분 (읽기 전용) | `state/daily/<date>/digest.md` + 알림 |
...
### 1.1 개정 — 2026-08-24 · 일간과 월간은 **다른 물건**이다. LLM 이 도는가로 갈린다

**위 §1 표를 지우지 않는다.** 아래는 그 표의 "일간" 행과 "월간" 행이 코드에서 실제로 무엇인지,
그리고 그 차이가 산출물의 **지위**에 무엇을 뜻하는지다.

단계 목록의 정본은 코드다:

| 케이던스 | 단계 (정본) | `research`(L3·LLM) |
|---|---|---|
| **일간** | `DAILY_STEPS = (scan, select, picks, diff, check, digest)` — `src/msa/pipeline/daily.py` | **없다.** 목록에 아예 존재하지 않는다 |
| **월간** | `MONTHLY_STEPS = (scan, select, research, ingest, picks, assemble, portfolio, report)` — `src/msa/pipeline/run.py` | **있다** (셋째 단계) |
| **주간** | `WEEKLY_STEPS = (scan, check, report)` | 없다 |

**일간에는 LLM 이 한 번도 안 돈다.** 전 단계가 결정론이다 (`CLAUDE.md` §4). 그것은 설계이지
결함이 아니다 — 일간은 **읽기 전용 후보 뷰**이고 결정 케이던스는 월간 그대로다.

**그 귀결을 명시한다 — 일간 다이제스트에 오른 테마 대부분은 가치함정 판정을 받은 적이 없다.**

| | |
|---|---|
| 가치함정 판별(`docs/04` 5축·하드 게이트)이 도는 곳 | **월간의 `research` 단계 하나뿐** |
| 그것도 도는 대상 | **상위 K=8 테마에만** (비용 통제, `docs/05` §5). 134 개 전부를 판정하는 경로는 설계상 없다 |
| 게다가 오늘의 기본값 | `--provider none` — `ANTHROPIC_API_KEY` 가 없어 **L3 를 부르지 않고** 사람 논지·직전 thesis 를 찾기만 한다 (이 문서 끝 "케이던스 오케스트레이터") |
| 그래서 명단 머리에 적히는 것 | `논지 없음 — L3 미실행 (msa research <theme> 또는 사람 논지 yaml)` (`src/msa/thesis.py` `NO_THESIS_NOTE`) |

> **명단에 올랐다는 것은 "함정이 아니라고 판정됨" 이 아니라 "판정 절차를 거치지 않음" 이다.**
> 빈 줄로 두지 않고 매번 그렇게 적는 이유가 이것이다 (`CLAUDE.md` §2 — 조용한 절단 금지).

**함께 바뀐 것 — 산출물의 성격.** 2026-08-24 에 L4 가 종목을 고르지 않게 됐으므로
(`docs/06` §5.1·§6.1 · `journal/2026-08-24-l4-selection-retired.md`), 일간·월간 어느 쪽이든
종목 관련 산출물은 **명단**이다: 하드 제외를 통과한 적격 종목 **전부**를 판단 재료 열과 함께
싣는다. `--per-theme`(기본 5)는 **표시 개수이지 선정 규칙이 아니다.** 명단에서 최종 종목과
진입 시점을 정하는 것은 **사람**이고, 그 자리는 §1 표의 "사람" 칸에 원래부터 있던 것이 아니라
2026-08-24 에 **L5 앞단으로 늘어난** 것이다 (`docs/00` §3 개정 절).

**임계·가중치는 하나도 옮기지 않았다.** 케이던스·`--top-k`(8)·`--per-theme`(5)도 그대로다.
