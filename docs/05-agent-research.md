...
### 6.9 판정일과 데이터 스냅샷일은 다른 물건이다 (2026-08-25)

**증상.** 6테마 병렬 라운드에서 **6테마 모두** `R_EVIDENCE_FUTURE` 로 저장을 거부당했다.
처음에는 "에이전트가 문서 발행일 대신 검색한 날짜를 적는다" 고 진단했다. **틀렸다.**
거부된 증거를 열어보니 실재하는 문서였다 — ZIM 2026년 2분기 실적 공시(2026-08-19),
Drewry World Container Index 주간 평가(2026-08-20). 날짜는 정확했다.

**원인.** `asof` 한 필드가 서로 다른 두 날짜를 겸하고 있었다.

| | 무엇 | 쓰임 |
|---|---|---|
| `data_asof` | 가격·재무가 끊긴 날 (= 쓰는 스캔의 날짜) | PIT 질의 상한 |
| `decision_date` | **판정을 내리는 날** | 증거가 미래인지 재는 기준 |

Sharadar 스토어가 2026-08-14 에서 끊겼고 판정은 2026-08-25 에 내렸다. 그 11일 사이의
문서는 **판정 시점에는 실재**한다. 미래 정보가 아니다. 스냅샷일로 재는 바람에 실재하는
증거가 전부 미래로 오판됐다. `docs/16` §4 의 결함 양식 ④(단위·공간 혼합)의 전형이다.

**수정.** `ResearchInputs.decision_date` 를 분리했다(기본 = 오늘). 증거의 미래 여부는
판정일로 재고, thesis 는 `inputs.data_asof` · `inputs.decision_date` · `inputs.data_lag_days`
셋을 모두 남긴다 — 합치면 어느 기준이었는지 다시 알 수 없기 때문이다. `decision_date` 가
`data_asof` 보다 앞서면 거부한다(존재하지 않는 데이터로 판정할 수 없다). 라운드 묶음
(`state/theses/<date>/`)은 **스캔 날짜 그대로** 둔다 — `scan`·`research`·`picks` 가 한 라운드를
공유하는 구조를 건드리지 않기 위해서다. 되돌려 재현할 때는 `--decision-date` 를 명시한다.

**규칙은 약해지지 않았다.** `R_EVIDENCE_FUTURE` 는 그대로 있고 기준만 옳은 날짜로 옮겼다.
판정일 이후 증거는 여전히 거부된다 — 회귀 테스트가 세 경우(스냅샷일 기준 거부 → 판정일
기준 통과 → 판정일 이후 재거부)를 모두 잡는다.

**같은 fixture 로 재판정한 결과** (역할 재호출 없이 $0):

| 테마 | 확신도 | 편입 | 비고 |
|---|---|---|---|
| `shipping_container` | 0.75 | **가능** | |
| `managed_care` | 0.75 | **가능** | |
| `home_improvement` | 0.60 | 불가 | 축1·축3 모두 적용 불가 → §2 조항이 닫음 |
| `health_it` · `offshore_drilling` · `media_streaming` | 0.45 | 불가 | 0.5 미달 |
| `retail_department` | 0.35 | 불가 | |

전부 0.45 로 붙던 것이 0.35~0.75 로 퍼졌다. 축1 결측이 확신도를 일률적으로 깎는다는
직전 진단은 이 결함이 만든 착시였다 — **임계값은 건드리지 않았다** (`CLAUDE.md` §1).
