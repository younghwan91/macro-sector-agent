...
## M3.6 · 설계 질문 1 — A(망각)의 집계 방식 (사전 등록된 검정)

> M3.5 의 판정(FAIL)이 가리킨 것은 가중치가 아니라 **집계 구조**일 수 있다 — `docs/02` 가 A 를 조건으로
> 서술하면서 §7 에서 가산한다. 질문과 절차는 `docs/12-design-question-a-block.md` 에 **실행 전에** 고정돼 있다.
> 이 마일스톤은 그 절차를 그대로 돌리는 것이며, 결과를 보고 후보나 임계를 고치지 않는다.

**완료 기준**
- [ ] `docs/12` §4.1 의 후보 S1(절대 게이트)·S2(풀/타이밍 2단) 를 **정의 그대로** 구현 (`scoreboard.py` 의
      대안 집계 함수 — 현행 S0 은 건드리지 않는다)
- [ ] `msa backtest l1 --structure S0|S1|S2` 로 같은 관문(주 창·12M·CI 하한 > 0) 적용, S1 의 G(t) < 20 인 달 제외 수 보고
- [ ] 시도 수 632 로 DSR 정산, `docs/backtest-l1.md` 에 **새 절**로 기록 (기존 절 불변)
- [ ] 결과별 조치는 `docs/12` §4.3 을 따른다 — 합격해도 자동 채택 아님. 채택은 사람이 별도 커밋으로
      (`02` §7·§8 개정 + `scoreboard.py` + README 강등 문구 + 저널 항목)
- [ ] D(밸류)를 풀 점수에 넣는 S2′ 는 **이 마일스톤에서 돌리지 않는다** (`docs/12` §5 — 새 시도이며 별도 결정)
