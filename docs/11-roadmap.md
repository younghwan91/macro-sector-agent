...
- [x] L2 인과 DAG 스펙 + 드라이버 목록 + 모순 감사 절차 — (2026-08-23 L2 제거, 아래 M4)
...
| 바뀐 것 | 자리 |
|---|---|
| L1·L4(당시 L2 포함)는 백테스트하고 walk-forward·DSR·PBO 관문을 붙인다 | `10` §1·§2, 새 마일스톤 M3.5 |
...
## M4 · L2 거시 DAG — **제거됨 (2026-08-23, 설계 질문 2 → C)**

> **폐기.** `docs/13` 의 설계 질문 2 에 대한 답은 "거시로 테마를 고르는 것의 OOS 예측력이 약하다"
> 였고, 같은 날 B 안(오버레이만)을 거쳐 **계층을 통째로 뺐다** — `src/msa/l2/`·`msa macro`·
> 월간 macro 단계·선정 오버레이·L3 순풍 항·저널 `l2_tailwind` 전부 삭제, DAG·감사 파일은
> `docs/archive/`. 이유·남는 것(FRED 는 L1 축 1·CPI 만)·되살리는 조건은
> `journal/2026-08-23-l2-removed.md` · `docs/03` 머리글. 아래 체크리스트는 **제거 시점의 상태를
> 기록으로** 남긴 것이다 — 더 진행하지 않는다.
...
## M4.5 · 설계 질문 2 — 거시(L2)를 순위에 다시 넣을 이유가 있는가 — **폐기 (2026-08-23, C 채택)**

> **폐기.** 이 증분 검정은 B 안(오버레이 유지)을 전제로 사전 등록한 것이다. 같은 날 C(L2 제거)를
> 채택하면서 재료(`msa macro` 의 tailwind 시계열)도 전제도 사라졌다 — 돌리지 않는다. 거시를
> 다시 들이려면 **이 등록을 되살리는 것이 아니라 새 설계 질문(`docs/14` 꼴)을 세우고 새로 사전
> 등록**한다 (`docs/13` §9 · `journal/2026-08-23-l2-removed.md`). 아래는 등록 당시 원문이다.
>
> (원문) 2026-08-23 B안 채택으로 L2 는 오버레이만(순위 가중 0). `docs/13` §5 가 **실행 전에** 고정한 증분 검정 —
...
- [x] **연결 — 배선** (2026-08-23 W1~W4) — L4 picks + thesis → L5 입력 묶음(`msa portfolio-inputs`, W1) ·
      ...
      **월간/주간 배선 — cron 이 `msa run monthly`/`msa run weekly` 를 호출**(W4: 스캔→상위 K→L3→
      적재→L4→L5 를 한 명령으로 — 거시 단계는 L2 제거로 빠짐, 단계별 ok/skipped/unavailable/failed 리포트 `state/runs/<date>/`)
