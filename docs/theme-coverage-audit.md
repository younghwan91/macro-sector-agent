...
## 5. ETF 프록시 vs 자체지수 12M 상관 — 기준 > 0.85 → **실행 (M3) · 17/44 통과 (38.6%)**

M3 의 `msa scan` 이 매 스캔마다 계산한다 (`state/scans/<date>/coverage.csv` 의 `etf_corr_12m`).
2026-08-14 기준: 프록시 보유 45개 중 계산 가능 44개 (`casinos_gaming` 의 `BJK` 는 벌크에 없음),
**17개가 > 0.85**, 중앙값 0.805, 최저 `infra_construction` 0.13(생존 3종목)·`agribusiness` 0.34·
`water_utility` 0.44·`waste_services` 0.45·`fertilizer_potash` 0.52. 최고는 `gold_miners` 0.97·
`banks_regional` 0.95·`copper_miners` 0.94·`silver_miners` 0.94. 이 검사는 **리포트 항목**이지
스캔을 막는 하드 실패가 아니다 — 다만 0.85 미만 버킷의 프록시는 자체지수를 대표하지 못하므로
`ew_vs_cw` 검증 외 용도로 쓰지 않는다.

M2 에서 미리 적어 둔 주의점은 그대로 유효하다. `etf_proxy` 가 `null` 인 버킷이 **134개 중 89개**로,
...
| 5. ETF 프록시 상관 | > 0.85 | **실행(M3)** — 17/44 통과, 중앙값 0.805. 리포트 항목 |
| 6. 시총 합계 추이 | 급변 시 로그 | **미실행** (첫 적재 — 기준선 확보) |

스캔을 막는 하드 실패(1·2)는 없다. 3·4 는 경고이며 해당 버킷의 신뢰도를 하향해 쓴다.
5 는 M3 에서 실행했고(매 스캔 재계산), 6 은 다음 적재부터 비교한다.
