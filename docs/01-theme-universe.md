...
- id: silver_miners                    # 스네이크케이스, 영구 불변 (저널이 이걸로 참조)
  ...
  physical_ref: {source: etf, symbol: SLV, kind: price}   # 실물/원자재 참조. 객체 또는 null (아래 규정)
...
**`physical_ref` 는 스칼라가 아니라 객체다.** 형식은 `{source, symbol, kind}` 이며 `source` 의
...
**`kind` 는 M3 에서 추가됐다 (`price` · `volume` · `nominal`).** 축 1 의 `unit_series` 는
참조가 가격인지 실물 물량인지에 따라 계산이 갈린다 (`04-value-trap.md` 축 1: 실물 소비량이 1순위,
매출/가격지수는 폴백). `volume` 이면 참조 그 자체가 `unit_series` 이고, `price` 면 동일 구성원 매출을
그것으로 나누며, `nominal`(달러 표시 판매액)은 CPI 로 실질화해 `volume` 처럼 쓴다. `kind` 가 없는
참조는 로더가 거부한다 — 추정하면 `ALTSALES`(판매 대수)를 가격으로 나누는 오류가 조용히 들어간다.
M2 확정본 45개의 분류: price 19 · volume 25 · nominal 1 (`state/themes.yaml`).
...
> **2026-08-24 정정.** 이전 판은 "**하나라도 실패하면 스캔이 진행되지 않는다**" 고 적었다.
> 사실이 아니었다 — 실제로 스캔을 막는 것은 아래 6개 중 **2개**뿐이다. 나머지는 보고만 하거나
> 아예 구현돼 있지 않다. 아래 "실제 동작" 열이 코드 기준이고, 없는 관문을 이 정정에서
> 구현하지는 않았다 (구현은 새 작업이다).

| 검사 | 기준 | 실제 동작 (2026-08-24 실측) | 실패 시 의미 |
|---|---|---|---|
| 미분류 시총 비율 | < 5% | **차단** — `l1/scan.py` `prepare_inputs` 가 `StoreError` 를 던진다 (`UNCLASSIFIED_MCAP_MAX`). 시총 결측 종목은 0 으로 덮이므로 관문은 통과 쪽으로만 틀린다 — 덮인 종목 수를 리포트에 함께 싣는다 | 버킷 정의에 큰 구멍. 새 산업이 생겼거나 매칭 규칙이 깨짐 |
| 중복 소속 티커 | 0개 | **차단** — `themes.load_themes` 의 `_assert_unique_owner` 가 `industry_match`·`include_tickers` 중복에 `ThemeSpecError`. 배정 자체가 티커당 단일 값이라 산출물에는 중복이 생길 수 없다 | 한 티커는 정확히 0 또는 1개 버킷. 중복은 L5 집중도 계산을 오염 |
| 구성원 < `min_constituents` 인 버킷 | 리포트에 경고 표기 | **경고** (선언대로) — 스코어보드 `small_sample` 플래그 + 리포트의 소표본 버킷 목록 | 중앙값 통계 신뢰 불가 — 스코어를 쓰되 신뢰도 하향 |
| ETF 프록시 vs 자체지수 12M 상관 | > 0.85 | **보고만** — `etf_proxy_corr` 가 계산해 `> 0.85` 비율을 리포트에 싣지만 어떤 임계도 강제하지 않는다 | 버킷 정의가 ETF 와 다른 것을 담고 있음 |
| 폐지 티커 포함 여부 | 자기이력 구간에 반드시 포함 | **미구현** — 검사 코드가 없다. 패널이 폐지 종목을 소급 포함하는 것(`l1/panel.py` 머리말 "지수 소급 구성")은 설계이지 검사가 아니다 | 빠지면 백분위가 낙관 방향으로 왜곡 (생존 편향) |
| 버킷별 시총 합계 추이 | 급변 시 로그 | **미구현** — 검사 코드가 없다 | 벤더 분류 변경 또는 매칭 규칙 회귀 |
