...
## 9. C 채택 — L2 제거 (2026-08-23, 같은 날 두 번째 결정)

§8 의 (B) 채택(58c006a) 뒤, 소유자가 **같은 날 (C) 를 결정**했다 — L2 를 오버레이로 남기는 대신
**계층 자체와 그것만을 위해 존재하던 모든 것을 뺀다.** 이 결정도 이 문서의 것이 아니라 별도
커밋의 것이며, 근거는 `journal/2026-08-23-l2-removed.md` 에 있다. 여기에는 **이 문서와의 관계**만
적는다.

**이 문서가 (C) 에 대해 적어 둔 것과 결정의 관계.**
- §4 표의 (C) 행은 비용을 셋 적었다 — `docs/03` §1 의 "읽을 수 있는 엣지"(L3 무효화 어휘) 상실,
  `credit_stress` 같은 낙폭 조건 상실, 되돌리기 비쌈. §6 은 (C) 를 권하지 않았다. **결정은 그 비용을
  알고 받아들인 것**이다 — 이 문서의 권고(§6)가 아니라 소유자의 판단이고, 이 문서는 그 판단을
  평가하지 않는다. 다만 §1 의 관측(순위 영향 0 · 드라이버 3/26 · `latest.json` 부재로 L3 +0.10 도
  미적용 · 부호 실측 29/451)은 "오늘 L2 가 실제로 한 일이 없다" 는 뜻이었고, §3 의 외부 증거는
  "거시는 섹터 선별기로서 표본 밖 근거가 약하다" 는 쪽이었다 — 결정이 선 자리는 거기다.
- **§5 의 사전 등록 검정(M4.5)은 폐기된다.** 그 검정은 "(B) 에서 (A) 로 돌아갈 이유가 있는가" 를
  F0/F1 ΔIC 로 답하는 절차였고, 재료(`msa macro` 의 tailwind 시계열)와 전제(오버레이 유지)가
  같이 사라졌다. **돌리지 않은 것이지 떨어진 것이 아니다** — §5 가 예고한 시도 수 646 은 실행되지
  않았으므로 장부는 632 그대로다(`docs/backtest-l1.md` §12). §5 는 원문 그대로 둔다(이 문서의
  규칙 — 고치지 않고 덧붙인다).
- **되살리는 길.** 거시를 다시 들이려면 이 문서의 §5 를 되살리는 것이 아니라 **새 설계 질문**을
  세운다(`docs/12`·이 문서의 꼴: 관측 → 선언의 근거 → 외부 증거 → 설계 공간 → 사전 등록). 그
  질문은 L2 가 있던 자리의 revert 가 아니라 "무엇을 재면 거시가 일한다고 할지" 부터 다시 쓴다.
  원 명세(`docs/03` 본문)·DAG(`docs/archive/macro-dag.yaml`)·감사 기록(`docs/archive/`)은 그
  출발점으로 남아 있다.

**무엇이 바뀌었는가 (요약 — 상세는 저널).** `src/msa/l2/` · `msa macro` · 월간 `macro` 단계 ·
선정 `hard_exclude` 오버레이 · L3 `MacroState`·프롬프트 거시 블록·확신도 거시 순풍 항(+0.10,
`docs/04` §4 개정: "입력이 사라졌다", 다른 항 불변) · 저널 진입 항목 `l2_tailwind` · L5 `tailwind` ·
FRED 드라이버 24종 목록 — 전부 삭제. **FRED 어댑터는 남는다** — L1 축 1 실물 참조
(`physical_ref.source == fred`)와 CPI 만(`docs/08` §3). `state/macro-dag.yaml`·`specs/macro-dag.example.yaml`·
`scripts/audit_dag.py`·`macro-dag-audit.md`·`macro-dag-sign-check.md` 는 `docs/archive/`.

이 문서는 여기서 다시 닫는다. 새 거시 설계 질문이 생기면 그 문서가 이 §9 를 링크한다.
