...
- 축 1 의 리그 카운트 10년 CAGR (Baker Hughes 연평균 시계열)은 확보하지 못했다. 다만 판정에 실제로 쓴 값(IEA 해상 생산 정체,
  IHS 계약 리그 −20% y/y)은 출처가 있으므로 yaml 은 `verified: true` 다 — 미확보인 것은 **대안 입력**이지 쓴 값이 아니다.
  확보되면 축 1 이 사망으로 바뀌어 자동 기각될 가능성이 높다 — 그래도 결론(편입 불가)은 같다.
