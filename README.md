...
> **상태: M3.5 까지 구현 (2026-08-23).** L0 데이터 계층(M1) · 테마 유니버스 134 버킷(M2) · L1 사이클
> 스캐너 `msa scan`(M3) · L1 백테스트 관문 `msa backtest l1`(M3.5) · 거시 DAG 선언(M4 의 YAML) 이 있다.
...
> 관찰 지표다** — 가중치는 바꾸지 않았다 (`CLAUDE.md` §1). 다음은 L2 런타임·L4·L5 다 — `docs/11-roadmap.md`. 데이터는 `~/data/us_micro.duckdb`(Sharadar) 이며
