...
flowchart TD
    ...
    L4["<b>L4 · 종목 선정</b><br/>IN: 게이트 통과 테마의 구성종목 전부<br/>OUT: 하드 제외 후 바벨 2~4 종목 (앵커/토크)"]
    L5["<b>L5 · 포트 구성기</b><br/>IN: 바벨 종목 + thesis + 자본<br/>OUT: 비중 · 사다리 3단 · 2단 스탑 · 시간 스탑 · TP 3단"]
...
| L4 (예: `commodity_chem`) | 구성원 35 → 상장 16 → 하드 제외 8 → **적격 8** → 바벨 4 | `uv run msa picks commodity_chem --asof 2026-08-14 --no-write --top 4` |
...
flowchart TD
    ...
    RS["<b>rank_score</b> = 0.40·S̃+0.40·T̃+0.20·M̃<br/>⚠ <b>표시용. 선정에 쓰이지 않는다</b>"]
    BB["<b>barbell.classify</b> — 실제 선정<br/>정렬: T̃ 내림차순<br/>앵커: S̃ ≥ 0.5 &amp; 한계생산자 아님 중 T̃ 상위 max(1, top//2)<br/>토크: S̃ &gt; 0.25 &amp; T̃ 계산됨 중 T̃ 상위 나머지"]
    OUT["<b>바벨 2~4 종목</b><br/>앵커 55~70% + 토크 30~45% (비중은 L5)"]
...
| **L4 종목 선정** | **가능** — S·T·M 이 전부 재무·가격 파생 | **2026-08-23 최초 실행 중. 결과 없음** |
...
**2026-08-23 현재 최초 실행이 진행 중이다.** 이 README 를 쓰는 시점에 **성과 수치·예상치는 없다.**

| # | 질문 | 합격 기준 |
|---|---|---|
| **Q1** | `rank_score` 가 테마 내 순서를 아는가 | 주 창 12M rank-IC 의 95% 블록부트스트랩 **CI 하한 > 0** |
| **Q2** | S·T·M 각 축 단독 IC | 축별 CI 하한 > 0 = "일한다" / 상한 < 0 = "반대로 일한다" / 0 포함 = "0". **관문이 아니다** |
| **Q3** | 하드 필터가 손실을 막았는가 | E1·E2·E3 각각 `제외군 − 통과군` 12M 초과수익 차의 CI **상한 < 0** |
| **Q4** | 지표 단독 | 판정 없음. 표로만 |
